...
if (!function_exists('show_column_value')) {
    ...
    function show_column_value($valueObject, $column, $return_format = '')
    {
        ...
        // Handle user ID columns
        if (in_array($column_name, ['user_id','resident_id','assigned_to', 'created_by', 'updated_by', 'deleted_by'])) {
        ...
        } elseif (strpos($column_type, 'enum(') === 0) {
            $return_text = !is_string($value) ? $value->displayName() : $value;
        } elseif ($column_type !== 'json' && $column_type !== 'enum' && \Illuminate\Support\Str::endsWith(strtolower($value), ['png', 'jpg', 'jpeg', 'gif', 'svg'])) {
